...
import { conversations, conversationMembers, messages, tokenTransfers } from '../db/schema.js';
...
conversationsRouter.get('/', async (req: AuthRequest, res) => {
  ...
  const result = memberships.map((membership) => serializeConversation(membership.conversation));
